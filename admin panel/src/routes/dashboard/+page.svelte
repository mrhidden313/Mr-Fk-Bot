<script>
    ...
    let token = '';
    let userEmail = '';
    let status = 'checking'; // 'checking', 'disconnected', 'starting', 'qr_ready', 'connected', 'error'
    let qrBase64 = null;
    let errorMsg = '';
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (data.error && data.error.includes('already active')) {
                    ...
                    errorMsg = data.error || `Failed to start session (${res.status})`;
                    status = 'error';
                    stopPolling();
                }
            }
    ...
    <div class="panel">
        ...
        {:else if status === 'disconnected' || status === 'not_found'}
            <div class="state-view">
                ...
                {/if}
                <button class="btn btn-primary" on:click={requestConnection} disabled={actionLoading}>
                    {#if actionLoading}
        ...
        {:else if status === 'qr_ready' && qrBase64}
            <div class="state-view">
                ...
                <div class="qr-timer">
                    <span class="dot-blink"></span> Waiting for scan...
                </div>
            </div>
        ...
        {:else if status === 'connected'}
            <div class="state-view">
                ...
                <div class="features-grid">
                    <div class="feature-chip">🔒 Anti-Delete ON</div>
                    <div class="feature-chip">👁 View-Once ON</div>
                    <div class="feature-chip">🤖 Auto-Reply ON</div>
                </div>

                <button class="btn btn-danger" on:click={disconnect} disabled={actionLoading}>
...
<style>
    .console { width: 100%; max-width: 560px; padding: 1rem; }
    ...
    .icon-green { background: rgba(16,185,129,0.15); border: 2px solid rgba(16,185,129,0.25); font-size: 2.5rem; font-style: normal; color: #34d399; }
    .icon-red { background: rgba(239,68,68,0.15); border: 2px solid rgba(239,68,68,0.25); font-size: 2.5rem; font-style: normal; color: #f87171; }
    ...
    /* Pulse rings animation for "starting" state */
    .pulse-rings {
        position: relative;
